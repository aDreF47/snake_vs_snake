...
class EstrategiaMinimax(EstrategiaIA):
    ...
    def minimax(
        self, estado: EstadoJuego, profundidad: int, es_maximizando: bool, motor_juego
        ...
        jugador_actual = (
            self.jugador
            if es_maximizando
            else ("rojo" if self.jugador == "azul" else "azul")
        )
        ...
        for movimiento, nuevo_estado in movimientos:
            valor, _ = self.minimax(
                nuevo_estado, profundidad - 1, not es_maximizando, motor_juego
            )
