...
class GestorEstado:
    ...
    def crear_estado_inicial() -> EstadoJuego:
        ...
        tablero = [[VACIO for _ in range(TABLERO_TAMAÑO)] 
                  for _ in range(TABLERO_TAMAÑO)]
    ...
    def es_posicion_valida(pos: Posicion) -> bool:
        ...
        return (0 <= pos.x < TABLERO_TAMAÑO and 
                0 <= pos.y < TABLERO_TAMAÑO)
    ...
    def aplicar_movimiento(estado: EstadoJuego, posicion: Posicion) -> MovimientoResult:
        ...
        if not GestorEstado.es_posicion_valida(posicion):
            return MovimientoResult(es_valido=False, 
                                  mensaje="Posición fuera del tablero")
        ...
        cabeza = (estado.cabeza_azul if estado.turno == AZUL 
                 else estado.cabeza_roja)
        if not cabeza:
        ...
        if posicion not in adyacentes:
            return MovimientoResult(es_valido=False,
                                  mensaje="Posición no adyacente a la cabeza")
