...
from core.interfaces import (
    TABLERO_TAMAÑO, AZUL, ROJO, Dificultad, Posicion, EstadoJuego
)
...
class PantallaDificultad:
    ...
    def dibujar(self) -> None:
        ...
        for dificultad, rect in self.botones.items():
            # Dibujar botón
            color = (COLORS['boton_hover'] if rect.collidepoint(pygame.mouse.get_pos())
                    else COLORS['boton'])
...
class PantallaJuego:
        ...
        for y in range(TABLERO_TAMAÑO):
            for x in range(TABLERO_TAMAÑO):
                ...
                if estado.tablero[y][x]:
                    ...
                    es_cabeza = (
                        (pos == estado.cabeza_azul and color == AZUL) or
    ...
    def convertir_pixel_a_casilla(self, pos_pixel: Tuple[int, int]) -> Optional[Posicion]:
        ...
        if (BOARD_OFFSET_X <= x < BOARD_OFFSET_X + TABLERO_TAMAÑO * CELL_SIZE and
            BOARD_OFFSET_Y <= y < BOARD_OFFSET_Y + TABLERO_TAMAÑO * CELL_SIZE):
...
class GestorInterfaz:
    ...
    def ejecutar_bucle_principal(
        self,
        callback_juego_iniciado: Callable,
        callback_movimiento: Callable
    ) -> None:
        """
        ...
        while ejecutando:
            ...
            if self.estado_actual == "dificultad":
                ...
                if self.pantalla_turno.manejar_eventos(eventos):
                    ...
                    self.pantalla_juego.establecer_callback_click(
                        callback_movimiento
                    )
